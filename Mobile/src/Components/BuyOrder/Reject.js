...
function Reject() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (data.length > 0) {
        setOrder(data.filter((data) => data.status === "Từ chối"));
      } else {
        setOrder([]);
      }
    } catch (error) {
      ...
      render: () => {
        return (
          <Box bg="gray.500" px="2" py="1" rounded="md" mb={10}>
            <Text color="white">Xoá đơn hàng thành công</Text>
          </Box>
        );
      },
    });
          ...
          order.map((order) => (
            <View key={order._id}>
              ...
              <View bg={Colors.white} pt={1} px={2}>
                <Text color="red.500">Đơn hàng bị từ chối</Text>
                <Text
                  bg={Colors.white}
                  color="gray.500"
                  fontSize={13}
              ...
              >
                <Pressable
                  ...
                  ml={2}
                >
                  <Text color="black">Xoá đơn hàng</Text>
                </Pressable>
              </View>
            </View>
          ))
